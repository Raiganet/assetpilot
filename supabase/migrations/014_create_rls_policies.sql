...
CREATE POLICY "Admins and supervisors can manage merchants"
  ON public.merchants FOR ALL
  USING (public.get_user_role(auth.uid()) IN ('administrator', 'supervisor'));
...
CREATE POLICY "Admins and warehouse staff can manage warehouses"
  ON public.warehouses FOR ALL
  USING (public.get_user_role(auth.uid()) IN ('administrator', 'warehouse'));
...
CREATE POLICY "Admins and warehouse staff can manage assets"
...
CREATE POLICY "Technicians can create withdrawals"
  ON public.withdrawals FOR INSERT
  WITH CHECK (public.get_user_role(auth.uid()) = 'technician');

CREATE POLICY "Admins and supervisors can update withdrawals"
  ON public.withdrawals FOR UPDATE
  USING (public.get_user_role(auth.uid()) IN ('administrator', 'supervisor'));

-- Receives policies
CREATE POLICY "Authenticated users can view receives"
  ON public.receives FOR SELECT
  USING (auth.role() = 'authenticated');

CREATE POLICY "Warehouse staff can manage receives"
  ON public.receives FOR ALL
  USING (public.get_user_role(auth.uid()) IN ('administrator', 'warehouse'));
...
CREATE POLICY "Admins and warehouse staff can manage quality checks"
...
CREATE POLICY "Admins and technicians can manage repairs"
  ON public.repairs FOR ALL
  USING (public.get_user_role(auth.uid()) IN ('administrator', 'technician'));
...
CREATE POLICY "Authenticated users can create chain of custody"
